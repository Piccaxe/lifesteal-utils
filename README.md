...
| **Health Indicators** | Color-coded health bar + HP number floating above each living entity within range, through walls. Shows the **real synced health** (and max-health, so lifesteal heart counts are correct) for both mobs and players. Optionally (`healthBarDamageEstimate`) a player's bar uses a damage-dealt estimate for instant feedback on your hits, then **continuously reconciles with the server's real health** each tick — so fire/lava, fall damage, other attackers, poison, and saturation regen are all reflected (it snaps to any mid-range reported value, ignoring a frozen full/zero). Range, players-only, and the estimate are configurable. |
...
| **Server whitelist** | Restrict the whole mod to specific servers. Turn the gate on and add server addresses; the mod then only runs on a server whose address matches an entry (singleplayer always allowed). Settings → **Servers** button, or `/piccaxeutils server on`, `server here`, `server add/remove <addr>`, `server list`. Off by default (mod runs everywhere). |
...
| **Hotbar key labels** | Small number on each hotbar slot showing which key selects it (follows the key remap live). On by default; `/piccaxeutils hotbar labels off`. |
| **Low-HP alert** | Tints the screen red (intensifying as you near death) and plays a warning sound when your health drops below a threshold (default 5 hearts). `/piccaxeutils lowhp on`, `lowhp hearts <1-20>`, `lowhp sound off`. Off by default. |
| **Potion warning** | Warns when watched potion effects (default **fire resistance** + **strength**) are about to run out: a chat + above-hotbar message at 10s left and an alert sound at 3s. Re-applying the potion re-arms it. `/piccaxeutils potionwarn on`, `potionwarn warnsec <n>`, `potionwarn soundsec <n>`, `potionwarn add/remove <effect>`, `potionwarn list`. Off by default. |
| **Cooldown HUD** | Tracks custom-enchant cooldowns from chat. Define rules (name + chat keyword + seconds); when a received message contains a keyword, that timer starts and counts down in a draggable HUD, flashing **READY** (with a ding) when up. `/piccaxeutils cooldown on`, `cooldown add <name> <seconds> <keyword…>`, `cooldown remove <name>`, `cooldown list`. e.g. `cooldown add Heavenly 300 heavenly is now on cooldown`. Off by default. |
| **Spawner ESP** | Outlines mob spawners through walls and colors them **green when activated** (a player within the activation radius, default 16 — vanilla) or red when idle, so you can see which spawners are live. `/piccaxeutils spawners on`, `spawners range <1-64>`. Off by default. |
| **Auto-builder** (experimental) | Walks to and places missing blocks of a loaded **Litematica** schematic — real movement, real look, real placements within normal reach (no reach/packet bypass). Reuses the built-in A* pathfinder. Honest limits: greedy/bottom-up, hotbar items only, no scaffolding/pillaring, abandons unreachable blocks; best for low/simple builds on solid ground. Toggle with the **Toggle Auto-Builder** keybind or `/piccaxeutils build` (`start`/`stop`). Needs Litematica + MaLiLib. **Movement automation is against most server rules — your call.** |
| **Music overlay** | On-screen "now playing" for whatever you're listening to (Spotify free/premium or any player), via Windows Now Playing (SMTC): **album art**, track (long titles **scroll**), artist, a **progress bar + elapsed/total time**, and ⏮ ⏯ ⏭ controls. Control with keybinds **Music: Previous / Play-Pause / Next** (bind in Controls) — the overlay shows each glyph's bound key. Draggable in the HUD editor; polls ~1s. `/piccaxeutils music on`, `music next|prev|playpause`. Windows-only, off by default. |
