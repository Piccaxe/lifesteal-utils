...
| **Music overlay** | On-screen "now playing" (track + artist) for whatever you're listening to (Spotify free/premium or any player), via Windows Now Playing (SMTC). Control with keybinds **Music: Previous / Play-Pause / Next** (bind in Controls) — the overlay shows ⏮ ⏯ ⏭ with your bound keys. Draggable in the HUD editor. `/piccaxeutils music on`, `music next|prev|playpause`. Windows-only, off by default. |
